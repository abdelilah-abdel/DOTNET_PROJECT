...
Security
Insights
Sentiment - Analysis - on - TripAdvisor - reviews / CODE / solution.py /
...
class ExamAssignment:
    ...
    def show_top_n_words(self, X_train, X_train_neg, X_train_pos):
        ...
        ax.bar(indices - (width / 2), (top_words_pos['count'] / len(X_train_pos)), width, color='#00800050',
               edgecolor='#00800095', linewidth=0.5, label='Positive class')
        ax.bar(indices + (width / 2), (top_words_neg['count'] / len(X_train_neg)), width, color='#80000050',
               edgecolor='#80000095', linewidth=0.5, label='Negative class')
    ...
    def perform_grid_search(self, X_train, y_train):
        ...
        with open('___gridsearch_da_definire.csv', 'w', encoding='utf-8') as file:
            print(
                f"CLASSIFIER,STEMMER,DUAL,STOP_WORDS_FLAG,MIN_LENGTH,NGRAM_1,NGRAM_2,MAX_DF,MIN_DF,C,LOSS,TOL,PENALTY,F1_WEIGHTED",
                file=file)
    ...
    def score_with_this_config(self,
        ...
        if classifier == 'gaussiannb':
            ...
            clf = RandomForestClassifier(n_estimators=50, max_depth=None, min_samples_split=2, max_features='auto',
                                         min_impurity_decrease=0.0)
        elif classifier == 'decisiontree':
            clf = DecisionTreeClassifier(max_depth=None, splitter='best', min_impurity_decrease=0.005,
        ...
        if pca_n_components != False and (
                classifier == 'gaussiannb' or classifier == 'multinomialnb' or classifier == 'complementnb'):
            print(f"SKIPPED")
            return
        ...
        with open('___gridsearch_da_definire.csv', 'a', encoding='utf-8') as file:
            print(
                f"{classifier},{AAAstemmer},{ABBdual},{stop_words_bool},{min_length},{ngram_range[0]},{ngram_range[1]},{max_df},{min_df},{C},{loss},{tol},{ABCpenalty},{score_mean}",
                file=file)
...
class MyTokenizer:
    ...
    # Constructor
    def __init__(self, no_numbers=True, min_length=1, clean_emoji=True, stop_words_bool=True, whitelist_stop_words=True,
                 stemmer=False):
    ...
    def __call__(self, doc):
        ...
        if self.stop_words_bool:
            split_doc = [word for word in split_doc if
                         len(word) > self.min_length and len(word) < 16 and self.stop_words_dict[word] != 1]
        else:
...
class StopWords:
    ...
    def __init__(self, whitelist=False):
        my_stop_words = set(
            ['http', 'www', 'per', 'che', 'con', 'era', 'del', 'della', 'tutto', 'come', 'ecc', 'etc', 'il', 'li', 'un',
             'una'])
